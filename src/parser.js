...
class Parser {
    ...
    log(...args) {
        ...
        if (debug) {
            console.log.apply(
                this,
                [chalk.cyan('i18next-scanner:')].concat(args)
            );
        }
    }
    error(...args) {
    ...
    parseFuncFromString(content, opts = {}, customHandler = null) {
        ...
        while ((r = re.exec(content))) {
            ...
            if (endsWithComma) {
                ...
                let code = matchBalancedParentheses(
                    content.substr(re.lastIndex)
                );
                ...
                try {
                    const syntax =
                        code.trim() !== '' ? parse('(' + code + ')') : {};
                    const props =
                        _.get(syntax, 'body[0].expression.properties') || [];
    ...
    parseTransFromString(content, opts = {}, customHandler = null) {
        ...
        const parseJSXElement = (node, code) => {
            ...
            const options = {
                ...tOptions,
                defaultValue:
                    defaultsString || nodesToString(node.children, code),
            ...
            if (Object.prototype.hasOwnProperty.call(attr, 'ns')) {
                if (typeof attr.ns !== 'string') {
                    this.log(
                        `The ns attribute must be a string, saw ${chalk.yellow(
                            attr.ns
                        )}`
                    );
                }
        ...
        try {
            ...
            if (transformOptions.filepath) {
                ...
                this.error(
                    `Unable to parse ${chalk.blue(component)} component:`
                );
                    ...
                    for (let i = 0; i < unusedKeys.length; ++i) {
                        ...
                        this.log(
                            `Removed an unused translation key { ${chalk.red(
                                ...
                                JSON.stringify(
                                    this.formatResourceLoadPath(lng, ns)
                                )
                            )}`
                        );
                    }
        ...
        lngs.forEach((lng) => {
            ...
            if (!_.isPlainObject(resLoad)) {
                ...
                this.error(
                    `${chalk.yellow(
                        ...
                        JSON.stringify(key)
                    )}, options=${chalk.yellow(JSON.stringify(options))}`
                );
                return;
            }
            Object.keys(keys).forEach((index) => {
                ...
                if (containsPlural) {
                    ...
                    if (
                        !containsContext ||
                        (containsContext && contextFallback)
                    ) {
                        resKeys.push(key);
                    }
                    if (containsContext) {
                        contextValues.forEach((contextValue) => {
                            resKeys.push(
                                `${key}${contextSeparator}${contextValue}`
                            );
                        });
                    }
                }
